...
def main() -> None:
    ...
    subparsers.add_parser("build", help="Search movies using BM25")
    ...
    bm25_tf_parser.add_argument(
        "b", type=float, nargs="?", default=BM25_B, help="Normalization Strength"
    )
    ...
    match args.command:
        ...
        case "bm25search":
            idx, results = bm25search_command(args.query, args.limit)
            count = 1
            for doc_id, score in results:
                doc = idx.docmap[doc_id]
                print(f"{count}. ({doc['id']}) {doc['title']} - Score: {score:.2f}")
                count += 1
        case _:
            parser.print_help()
